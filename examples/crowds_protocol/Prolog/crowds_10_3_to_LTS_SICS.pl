...
trans_crowds_10_3(S1,S2,P) :-
    transition(S1,_,_,S2),
    (transition(S1,'Is_a_good_member'(pred_true),_,S2) -> P is 0.909
        ; transition(S1,'Is_a_good_member'(pred_false),_,S2) -> P is 0.091
        ; transition(S1,'Forward_or_deliver'(pred_true),_,S2) -> P is 0.8
        ; transition(S1,'Forward_or_deliver'(pred_false),_,S2) -> P is 0.2
        ; transition(S1,'Record'(_),_,S2) -> P is 0.1
        ; P is 1.0
    ).
