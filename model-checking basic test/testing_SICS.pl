...
:- module(testing_SICS,[choose_model/1,global_test/0/*,test_loop/0,test_big/0*/]).

:- use_module(dtmc_model_checking_SICStus,[sat/2]).
...
test_loop_until_bounded :-
    (findall(
        ...
        ) -> print('Until-bounded formula suceeds\n')
...
test_loop_eventually :- 
    ...
    (compare_floats_list(
        ...
        ) -> print('Eventually formula suceeds\n')
...
test_big_until_bounded :-
    (sat(
        prob_formula(
            ...
            0.95703125,
            u(true,5,p(elect))),
        0) -> print('Until-bounded formula suceeds\n')
            ; print('Until-bounded formula fails\n')
    ).
