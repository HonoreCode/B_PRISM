...
:- module(dtmc_model_checking_SICS,[sat/2]).
...
sat_not(p(Property),E) :- \+(prop(E,Property)).
...
prob_calc(u(F,G),E,P_phi) :- 
    retractall(table_prob0(_E)),
    retractall(table_prob1(_S)),
...
prob1(F,G,E1) :- 
    sat(F,E1),
    assert(table_prob1(E1)),
    trans(E1,E2,_P),
    \+ table_prob1(E2),
    prob1(F,G,E2),!.

search_prob1(F,G,E) :- 
    retractall(table_prob1(_E)),
    prob1(F,G,E).

% P_vect is the vector of non-null probabilities 
% for the until formula
prob_calc_u1(F,G,List_S,P_vect) :- 
    findall(S,(state(S),prob0(F,G,S)),List_S), /*prob0 faster than search_prob0*/
